...
import { transformCategory, transformPostList } from '../utils/apiTransformer';
  ...
  const loadCategory = async () => {
    try {
      ...
      if (response.success && response.data) {
        setCategory(transformCategory(response.data));
      } else {
        setError(response.error || '分类不存在');
      }
    } catch (err: any) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-slate-800 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">分类不存在</h2>
          <p className="text-gray-600 dark:text-gray-400 mb-6">{error || '抱歉，该分类不存在或已被删除'}</p>
          <Link
            to="/"
      ...
      <div className="relative bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-600 text-white">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
            {/* 分类图标 */}
            <div
              className="w-24 h-24 md:w-32 md:h-32 rounded-2xl flex items-center justify-center text-4xl md:text-5xl font-bold shadow-lg"
            ...
            <div className="flex-1">
              ...
              {category.description && (
                <p className="text-white/80 text-lg mb-4 max-w-2xl">{category.description}</p>
              )}
              ...
              <div className="flex flex-wrap gap-4 md:gap-6">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-white/80">{formatNumber(category.postCount)} 篇文章</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {posts.length === 0 ? (
          <div className="text-center py-16 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200 dark:border-slate-700">
            ...
            <p className="text-gray-600 dark:text-gray-400">该分类下还没有发布任何文章</p>
          </div>
        ) : (
          <>
